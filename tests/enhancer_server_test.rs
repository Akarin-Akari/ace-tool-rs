...
async fn test_set_bind_addr() {
    ...
    // Before start, port should still be None (no actual binding yet)
    assert!(server.get_port().await.is_none());
}
...
async fn test_start_with_custom_addr() {
    ...
    let port = server.get_port().await;
    assert!(port.is_some());
    assert_ne!(port.unwrap(), 0, "Port should be assigned by OS, not 0");
...
async fn test_set_bind_addr_concurrent_with_start() {
    ...
    let port = server.get_port().await;
    assert!(port.is_some());
    assert_ne!(port.unwrap(), 0, "Port must be non-zero after start() completes");
...
async fn test_concurrent_start_calls() {
    ...
    let port = server.get_port().await;
    assert!(port.is_some());
    assert_ne!(port.unwrap(), 0, "Port must be non-zero after concurrent starts");
}
